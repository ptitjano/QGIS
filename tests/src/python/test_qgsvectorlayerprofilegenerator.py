...
class TestQgsVectorLayerProfileGenerator(QgisTestCase):
    ...
    def testPointGenerationAbsolute(self):
        vl = QgsVectorLayer(
            os.path.join(unitTestDataPath(), "3d", "points_with_z.shp"), "trees"
        )
        ...
        self.assertCountEqual(
            [f.geometry.asWkt(-2) for f in features],
            [
                "Point Z (-347400 6632600 300)",
                "Point Z (-347500 6632700 300)",
                "Point Z (-346400 6632300 200)",
            ],
        )
        ...
        self.assertCountEqual(
            ...
            [
                "Point Z (-347530 6632710 270)",
                "Point Z (-347390 6632650 270)",
                "Point Z (-346400 6632270 220)",
            ],
        )
    ...
    def testPointGenerationMultiPoint(self):
        ...
        f.setGeometry(
            QgsGeometry.fromWkt(
                "MultiPoint Z(322069 129893 89.1, 322077 129889 90.2, 322093 129888 92.4)"
            )
        )
    ...
    def testLineGenerationAbsolute(self):
        ...
        for line in [
            "LineStringZ(322006 129874 12, 322008 129910 13, 322038 129909 14, 322037 129868 15)",
    ...
    def testLineGenerationFollowingLinestringExactly(self):
        ...
        for line in [
            "MultiLineString ((872381.44973557780031115 6035318.57090197317302227, 868258.19322114891838282 6039288.30190788581967354, 870254.02483185648452491 6048565.62906535062938929, 884224.8461068095639348 6045999.55985158402472734, 876087.99415546329692006 6035472.0964104887098074))"
        ]:
            f = QgsFeature()
            f.setGeometry(QgsGeometry.fromWkt(line))
            self.assertTrue(vl.dataProvider().addFeature(f))
    ...
    def testLineGenerationFollowingLinestringZExactly(self):
        ...
        for line in [
            "MultiLineStringZ ((872381.44973557780031115 6035318.57090197317302227 690, 868258.19322114891838282 6039288.30190788581967354 705, 870254.02483185648452491 6048565.62906535062938929 680, 884224.8461068095639348 6045999.55985158402472734 700, 876087.99415546329692006 6035472.0964104887098074 710))"
        ]:
            f = QgsFeature()
            f.setGeometry(QgsGeometry.fromWkt(line))
            self.assertTrue(vl.dataProvider().addFeature(f))
    ...
    def testDataDefinedExtrusionOffset(self):
        vl = QgsVectorLayer(
            "PolygonZ?crs=EPSG:27700&field=extrusion:int&field=offset:int",
            "lines",
            "memory",
        )
        ...
        vl.elevationProperties().dataDefinedProperties().setProperty(
            QgsMapLayerElevationProperties.Property.ExtrusionHeight,
            QgsProperty.fromField("extrusion"),
        )
        vl.elevationProperties().dataDefinedProperties().setProperty(
    ...
    def testSnappingPoints(self):
        ...
        curve = QgsLineString()
        curve.fromWkt(
            "LineStringZ (322021.96201738982927054 129896.83061585001996718 0, 322116.8371042063809 129880.94244341662852094 0)"
        )
        req = QgsProfileRequest(curve)
    ...
    def testSnappingPolygons(self):
        ...
        curve = QgsLineString()
        curve.fromWkt(
            "LineStringZ (321944.79089414176996797 129899.10035476912162267 0, 321818.13946245843544602 129991.70570266660070047 0)"
        )
        req = QgsProfileRequest(curve)
    ...
    def testIdentifyPoints(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "delta": 0.9654154289752465,
                    "distance": 15.895441865142377,
                    "elevation": 14.360847359216885,
                    "id": 1,
                }
            ],
        )
        ...
        self.assertEqual(
            ...
            [
                {
                    "delta": 0.3133775679752489,
                    "distance": 55.279676009112876,
                    "elevation": 16.14137478571788,
                    "id": 2,
                }
            ],
        )
    ...
    def testIdentifyPolygons(self):
        ...
        self.assertEqual(
            ...
            [
                {
                    "delta": 0.3909814462196946,
                    "distance": 27.377976839618572,
                    "elevation": 2.0,
                    "id": 2,
                }
            ],
    ...
    def testIdentifyExtrudedPolygons(self):
        ...
        self.assertEqual(
            res[0].results(),
            [{"delta": 0.0, "distance": 35.0, "elevation": 16.0, "id": 2}],
        )
    ...
    def testRenderProfileDataDefinedProperties(self):
        vl = QgsVectorLayer(
            "PolygonZ?crs=EPSG:27700&field=color:string", "lines", "memory"
        )
        vl.setCrs(QgsCoordinateReferenceSystem())
        self.assertTrue(vl.isValid())
        ...
        fill_symbol[0].dataDefinedProperties().setProperty(
            QgsSymbolLayer.Property.PropertyFillColor, QgsProperty.fromField("color")
        )
        ...
        self.assertTrue(
            self.image_check(
                "vector_polygon_data_defined_symbology",
                "vector_polygon_data_defined_symbology",
                res,
            )
        )
    ...
    def testRenderProfileAsSurfaceLines(self):
        ...
        res = plot_renderer.renderToImage(400, 400, 0, curve.length(), 0, 14)
        self.assertTrue(
            self.image_check("vector_lines_as_surface", "vector_lines_as_surface", res)
        )
    ...
    def testRenderProfileAsSurfaceLinesWithMarkers(self):
        ...
        marker_symbol = QgsMarkerSymbol.createSimple(
            {"name": "square", "size": 4, "color": "#00ff00", "outline_style": "no"}
        )
        vl.elevationProperties().setRespectLayerSymbology(False)
    ...
    def testRenderProfileAsLineWithHoledDtm(self):
        rl = QgsRasterLayer(
            os.path.join(unitTestDataPath(), "3d", "dtm_with_holes.tif"), "DTM"
        )
        self.assertTrue(rl.isValid())
        ...
        self.assertTrue(
            self.image_check(
                "vector_lines_as_line_with_holed_dtm",
                "vector_lines_as_line_with_holed_dtm",
                res,
            )
        )
    ...
    def testRenderProfileAsSurfaceFillBelowWithHoledDtm(self):
        ...
        rl.elevationProperties().setEnabled(True)
        rl.elevationProperties().setProfileSymbology(
        ...
        self.assertTrue(
            self.image_check(
                "vector_lines_as_fill_below_surface_with_holed_dtm",
                "vector_lines_as_fill_below_surface_with_holed_dtm",
                res,
            )
        )
    ...
    def testRenderProfileAsSurfaceFillBelowLimit(self):
        ...
        vl.elevationProperties().setElevationLimit(1)
        fill_symbol = QgsFillSymbol.createSimple(
            {"color": "#ff00ff", "outline_style": "no"}
        )
        vl.elevationProperties().setRespectLayerSymbology(False)
    ...
    def testRenderProfileAsSurfaceFillAbove(self):
        ...
        vl.elevationProperties().setType(Qgis.VectorProfileType.ContinuousSurface)
        vl.elevationProperties().setProfileSymbology(
            Qgis.ProfileSurfaceSymbology.FillAbove
        )
        fill_symbol = QgsFillSymbol.createSimple(
            {"color": "#ff00ff", "outline_style": "no"}
        )
    ...
    def testRenderProfileAsSurfaceFillAboveLimit(self):
        ...
        vl.elevationProperties().setElevationLimit(10)
        fill_symbol = QgsFillSymbol.createSimple(
            {"color": "#ff00ff", "outline_style": "no"}
        )
        vl.elevationProperties().setRespectLayerSymbology(False)
    ...
    def testRenderProfileSymbolWithMapUnits(self):
        ...
        self.assertTrue(
            self.image_check(
                "vector_profile_map_units", "vector_profile_map_units", res
            )
        )
    ...
    def testRenderProfileSymbolWithMapUnitsTolerance(self):
        ...
        self.assertTrue(
            self.image_check(
                "vector_profile_map_units_tolerance",
                "vector_profile_map_units_tolerance",
                res,
            )
        )
    ...
    def testRenderLayerSymbology(self):
        ...
        self.assertTrue(
            self.image_check(
                "vector_polygon_layer_symbology", "vector_polygon_layer_symbology", res
            )
        )
    ...
    def testRenderProfileWithSubsections(self):
        ...
        vl.elevationProperties().setClamping(Qgis.AltitudeClamping.Absolute)
        vl.elevationProperties().setRespectLayerSymbology(False)
        line_symbol = QgsLineSymbol.createSimple({"color": "#ff00ff", "width": "1.2"})
        line_symbol.setWidthUnit(Qgis.RenderUnit.MapUnits)
        vl.elevationProperties().setProfileLineSymbol(line_symbol)
        vl.elevationProperties().setCustomTolerance(tolerance)
    ...
    def testPolyhedralSurfaceGenerationFeature(self):
        ...
        vl = QgsVectorLayer(
            "PolyhedralSurfaceZ?crs=epsg:27700", "polyhedral_surface", "memory"
        )
        self.assertTrue(vl.isValid())
    ...
    def testVerticalLineGenerationFeatureTolerance(self):
        ...
        vl_feature.setGeometry(
            QgsGeometry.fromWkt(
                "LineStringZ(321777 129912 1, 321777 129912 -22, 321777 129912 1))"
            )
        )
        self.assertTrue(vl.dataProvider().addFeature(vl_feature))
    ...
    def testProfileTransformGDA2020toAVWS(self):
        ...
        available_operations = QgsDatumTransform.operations(vl.crs3D(), profile_crs)
