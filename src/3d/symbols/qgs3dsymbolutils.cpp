...
std::unique_ptr<QgsAbstract3DSymbol> Qgs3DSymbolUtils::create3DSymbolFrom2D( const QgsVectorLayer *vLayer, const QgsSymbol *symbol2D, const QgsRenderContext &context )
{
  ...
  else if ( symbol3D->type() == "polygon"_L1 )
  {
    // polygon geometry type - retrieve edges
    QgsPolygon3DSymbol *polygonSymbol3D = dynamic_cast<QgsPolygon3DSymbol *>( symbol3D.get() );

    if ( symbol2D->type() == Qgis::SymbolType::Fill )
    {
      const QgsFillSymbol *fillSymbol = dynamic_cast<const QgsFillSymbol *>( symbol2D );
      bool hasEdges = false;
      QColor edgeColor;
      double edgeWidth;
      Qgis::RenderUnit edgeUnit;
      polygonSymbol3D->setEdgesEnabled( false );
      for ( int i = 0; i < fillSymbol->symbolLayerCount(); ++i )
      {
        const QgsSymbolLayer *symbolLayer = fillSymbol->symbolLayer( i );
        if ( const QgsSimpleFillSymbolLayer *fillSymbolLayer = dynamic_cast<const QgsSimpleFillSymbolLayer *>( symbolLayer ) )
        {
          if ( fillSymbolLayer->strokeStyle() != Qt::NoPen )
          {
            hasEdges = true;
            edgeColor = fillSymbolLayer->strokeColor();
            edgeWidth = fillSymbolLayer->strokeWidth();
            edgeUnit = fillSymbolLayer->strokeWidthUnit();
          }
        }
        else if ( const QgsSimpleLineSymbolLayer *lineSymbolLayer = dynamic_cast<const QgsSimpleLineSymbolLayer *>( symbolLayer ) )
        {
          hasEdges = true;
          edgeColor = lineSymbolLayer->strokeColor();
          edgeWidth = lineSymbolLayer->width();
          edgeUnit = lineSymbolLayer->widthUnit();
          qDebug() << "je suis simple line" << edgeColor;
        }

        if ( hasEdges && edgeColor.isValid() )
        {
          const double edgeWidthPixels = std::max( 1.0, context.convertToPainterUnits( edgeWidth, edgeUnit ) );
          polygonSymbol3D->setEdgesEnabled( true );
          polygonSymbol3D->setEdgeWidth( static_cast<float>( edgeWidthPixels ) );
          polygonSymbol3D->setEdgeColor( edgeColor );
          break;
        }
      }
    }
