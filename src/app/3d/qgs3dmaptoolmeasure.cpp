...
void Qgs3DMapToolMeasure::activate()
{
  ...
  mRubberBand = std::make_unique<QgsRubberBand3D>( mCanvas->scene(), rubberbandType );
...
void Qgs3DMapToolMeasure::mouseMoveEvent( QMouseEvent *event )
{
  ...
  const QgsPoint pointMap = Qgs3DUtils::screenPointToMapCoordinates( event->pos(), mCanvas->size(), mCanvas->cameraController(), mCanvas->mapSettings(), mCanvas->scene()->centerDepth() );
