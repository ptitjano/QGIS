  ...
  , mLayerTreeBridge( new QgsLayerTreeRegistryBridge( mLayerTree.get(), mProfile->layout() ? mProfile->layout()->project() : QgsProject::instance(), this ) )
{
  ...
  mSubsectionsSymbolButton->setDefaultSymbol( QgsPlotDefaultSettings::subSectionsSymbol() );
  ...
  connect( mSubsectionsActivateCheck, &QCheckBox::toggled, this, [=] {
    ...
    QgsLineSymbol *subSectionsSymbol = subsectionsActivated ? mSubsectionsSymbolButton->clonedSymbol<QgsLineSymbol>() : nullptr;
    mProfile->setSubsectionsSymbol( subSectionsSymbol );
...
void QgsLayoutElevationProfileWidget::copySettingsFromProfileCanvas( QgsElevationProfileCanvas *canvas )
{
  ...
  QgsLineSymbol *subSectionsSymbol = canvas->subsectionsSymbol() ? canvas->subsectionsSymbol() : nullptr;
...
void QgsLayoutElevationProfileWidget::setGuiElementValues()
{
  ...
  mSubsectionsActivateCheck->setChecked( mProfile->subsectionsSymbol() != nullptr );
